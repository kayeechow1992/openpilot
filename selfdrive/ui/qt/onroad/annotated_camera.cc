...
void AnnotatedCameraWidget::updateState(const UIState &s)
{
  ...
  // update Camera View
  cameraView = s.scene.camera_view;
}
...
void AnnotatedCameraWidget::drawHud(QPainter &p)
{
  ...
  if (has_us_speed_limit && speedLimitStr.size() >= 3)
    set_speed_size.rwidth() = 223;
...
void AnnotatedCameraWidget::paintGL()
{
  ...
  // draw camera frame
  {
    ...
    CameraWidget::setStreamType(cameraView == 1 ? VISION_STREAM_DRIVER : cameraView == 3 || wide_cam_requested ? VISION_STREAM_WIDE_ROAD
                                                                                                               : VISION_STREAM_ROAD);
